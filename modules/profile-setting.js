angular.module('app-module',['ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,$q,bui) {
			...
			username_is_unique(scope).then(function(res) {
				...
				if (!scope.settings.info.not_unique) {
					...
					$http({
						...
						growl.show('btn btn-default',{from: 'top', amount: 60},'Your username has been updated.');						
		...
		self.security.update = function(scope) {
			...
			if (!validate(scope,'security') && scope.settings.security.alert.opw.correct && scope.settings.security.alert.password.correct && scope.settings.security.alert.password.okMin) {
				...
				$http({
					...
					growl.show('btn btn-default',{from: 'top', amount: 60},'Your password has been updated. Please logout then login again.');						
